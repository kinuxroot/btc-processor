...
LoggerType& getLogger() {
    ...
    static auto logger = LoggerFactory<Level::Debug>::createLogger("Merge address balance", std::make_tuple(
        StreamHandler<Level::Debug>(formatRecord),
        FileHandler<Level::Debug>::create("logs/btc_merge_address_balance.log", formatRecord)
    ));
